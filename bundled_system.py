...
def run_submission():
    ...
    install(verbose=False) # advanced_modules install

    try:
        import aimo
        env = aimo.make_env()
        iter_test = env.iter_test()
        print("AIMO API Connected.")
        for (test, sample_sub) in iter_test:
            problem = test.iloc[0]['problem']
            # Use the patched run from discovery_engine_v5
            result = run(problem, quiet=True, json_out=True)
            ans = int(result.get('ans', 0))
            sample_sub['answer'] = ans % 100000
            env.predict(sample_sub)
    except Exception as e:
        print(f"API Error: {e}. Fallback to manual.")
        # Fallback for authoring
        ...
        if os.path.exists(test_path):
            ...
            for _, r in df.iterrows():
                res = run(r['problem'], quiet=True, json_out=True)
                ids.append(r['id']); ans.append(int(res.get('ans', 0)) % 100000)
